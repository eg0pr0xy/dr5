...
const Panel: React.FC<PanelProps> = ({ title, footer, collapsedOnXs, children }) => {
  ...
  return (
    <section className="surface-shell relative border border-current bg-transparent flex flex-col min-h-0 overflow-hidden">
      <div className="surface-scanline pointer-events-none" />
      {title !== undefined && (
        <div className="flex items-center justify-between px-2 py-1 border-b border-current select-none">
          <div className="surface-readout text-[10px] uppercase tracking-[0.3em] readout">{title}</div>
          {collapsedOnXs && (
            <div className="text-[10px] cursor-pointer" onClick={toggle}>{open ? '[-]' : '[+]'}</div>
          )}
        </div>
      )}
      <div className="surface-pane flex-1 min-h-0 content-scroll px-2 py-2">
